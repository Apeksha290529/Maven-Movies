...
SELECT INV.store_id,F.rating,COUNT(INVETORY_ID) AS NUMBER_OF_COPIES                  
...
SELECT F.FILM_ID, F.TITLE AS FILM_NAME,C.NAME AS CATEGORY_NAME,LANG. NAME AS LANG_NAME
...
SELECT F.TITLE,R.RENTAL_ID,sum(p.amount)as revenue
from rental as R LEFT JOIN INVENTORY AS INV
ON R.INVENTORY_ID = INV.INVENTORY_ID LEFT JOIN FILM AS F
ON INV.FILM_ID= F.FILM_ID LEFT JOIN PAYMENT AS P
ON R.INVENTORY_ID=P.RENTAL_ID
GROUP BY F.TITLE
ORDER BY POPULARITY DESC;
...
SELECT S.STORE_ID,SUM(AMOUNT) AS REVENUE_PER_STORE
...
select CUSTOMER_ID,count(*) AS NUMBERS_OF_RENTALS
FROM RENTAL
group by CUSTOMER_ID
HAVING NUMBERS_OF_RENTALS>30;
 
select R.CUSTOMER_ID,COUNT(*) AS NUMBERS_OF_RENTALS,C.FIRST_NAME,C.LAST_NAME,C.EMAIL
...
FROM RENTAL
group by CUSTOMER_ID
HAVING NUMBERS_OF_RENTALS>30)AS MORE_THAN_30);
